    ...
    <div className="space-y-6 pb-16 max-w-7xl mx-auto">
      <section className="flex flex-col gap-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-foreground/50">{traduzir('cartoesSub')}</p>
          </div>
          <button onClick={abrirModalNovo} className="bg-foreground text-background px-6 py-3 rounded-full font-bold text-sm flex items-center justify-center gap-2 hover:scale-[1.02] shadow-md focus-ring">
            <Plus size={18} /> {traduzir('novoCartao')}
          </button>
        </div>
        <MonthCarousel />
      </section>
      ...
      {loadingCartoes ? (
        ...
        <div className="glass rounded-3xl p-16 text-center flex flex-col items-center border-dashed border-2">
          ...
          <h3 className="text-xl font-bold mb-2">{traduzir('nenhumCartao')}</h3>
          <button onClick={abrirModalNovo} className="bg-foreground text-background px-6 py-2.5 mt-4 rounded-full font-bold text-sm">
            {traduzir('adicionarCartao')}
          </button>
        </div>
      ) : (
                  ...
                  <div className="w-full h-44 rounded-[1.3rem] p-5 flex flex-col justify-between text-white shadow-md relative overflow-hidden" style={{ backgroundColor: cartao.cor }}>
                    ...
                    <div className="flex justify-between items-start relative z-10">
                      <div>
                        ...
                        <p className="text-xs opacity-70 mt-0.5 flex items-center gap-1"><Calendar size={12} /> {traduzir('venceDia')} {cartao.diaVencimento}</p>
                      </div>
                    ...
                    <div className="relative z-10 flex justify-between items-end">
                      <div>
                        <p className="text-[10px] uppercase opacity-70 mb-0.5 font-bold tracking-widest">{traduzir('limiteDisponivel')}</p>
                  ...
                  <div className="p-5 pt-4 space-y-4">
                    <div className="flex items-center justify-between p-3 rounded-xl bg-foreground/5 border border-border">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold uppercase tracking-wider text-foreground/60">{traduzir('faturaPrevista')}</span>
                      </div>
                    ...
                    <div className="space-y-1.5">
                      <div className="flex justify-between items-end">
                        <span className="text-[10px] font-bold text-foreground/50 uppercase tracking-wider">{traduzir('usoLimiteTotal')}</span>
                      ...
                      <div className="flex justify-between text-[10px] font-bold text-foreground/40">
                        <span>{traduzir('comprometido')}: {formatarMoeda(limiteUsado)}</span>
                        <span>{traduzir('total')}: {formatarMoeda(cartao.limiteTotal)}</span>
                      </div>
                    </div>
                  </div>
                </motion.div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }} className="w-full max-w-md bg-background border border-border rounded-3xl p-6 shadow-2xl relative z-10 glass">
              <div className="flex justify-between items-center mb-6">
                <h3 className="font-display text-xl font-bold">{cartaoEmEdicao ? traduzir('editarCartao') : traduzir('novoCartao')}</h3>
              ...
              <form onSubmit={handleSalvarCartao} className="space-y-4">
                <div>
                  <label className="text-xs font-bold text-foreground/60 block mb-1">{traduzir('Nome')}</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-bold text-foreground/60 block mb-1">{traduzir('Limite Total (R$)')}</label>
                  ...
                  <div>
                    <label className="text-xs font-bold text-foreground/60 block mb-1">{traduzir('Vencimento (Dia)')}</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-bold text-foreground/60 block mb-1">{traduzir('Bandeira')}</label>
                  ...
                  <div>
                    <label className="text-xs font-bold text-foreground/60 block mb-1">{traduzir('Cor')}</label>
                ...
                <button type="submit" className="w-full mt-2 bg-primary text-primary-foreground font-bold text-sm py-3.5 rounded-xl hover:scale-[1.02] transition-all">
                  {cartaoEmEdicao ? traduzir('editarCartao') : traduzir('adicionarCartao')}
                </button>
              </form>
            </motion.div>
          </div>
